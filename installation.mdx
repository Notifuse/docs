...
You have two options to deploy Notifuse:
...
This will start Notifuse on port `8080` with a PostgreSQL database. You'll still need to configure the [required environment variables](#required-variables) in a `.env` file or directly in the `docker-compose.yml`.
...
**Note**: You'll need to set the [required environment variables](#required-variables) and provide your own PostgreSQL database.

### PASETO keys

You can generate PASETO keys with https://paseto.notifuse.com

Alternatively, you can generate keys manually using the built-in keygen command:
...
You can set up a cron service in `docker-compose.yml` using the snippet below:
