...
| ------------------------------------------ | -------- | ------------------------------------------ |
| `email`                                    | String   | Contact's email address — the contact's unique identifier (**Required**) |
| `external_id`                              | String   | Optional reference to the contact in your own system. Not unique — see [Contact Identity](#contact-identity-and-upsert-behavior) |
...
<Warning>
  Preserve opt-out statuses when re-creating subscriptions. If the old contact had unsubscribed,
  bounced, or complained on a list, do **not** re-subscribe the new email address to that list:
  the new contact starts with a clean slate, and blindly re-subscribing silently resets an
  opt-out — a consent violation (GDPR / CAN-SPAM) that can hurt your sender reputation.
</Warning>
