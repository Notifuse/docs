...
| Variable          | Description                                                      |
| ----------------- | ---------------------------------------------------------------- |
| `ROOT_EMAIL`      | Root administrator email (e.g., `admin@yourcompany.com`). Accepts multiple comma/semicolon-separated emails to grant root to several administrators (e.g., `alice@yourcompany.com,bob@yourcompany.com`); matching is case-sensitive. |
...
<Note>
  **Multiple root users**: `ROOT_EMAIL` can hold more than one email
  ...
  there as a list or via the `ROOT_EMAIL` environment variable.
</Note>
...
| `BROADCAST_DATA_FEED_ALLOW_PRIVATE_HOSTS` | Allow broadcast [data feeds](/features/broadcast-campaigns) to fetch from private, loopback, and link-local addresses. This **disables SSRF protection** for data-feed requests — only enable it in trusted deployments that intentionally fetch feeds from your internal network (e.g., `true`). | `false` |
|                                     |
| **Privacy Settings**                |
...
|                                     |
| **Single Sign-On (OIDC)** — see [SSO guide](/self-hosting/sso) | | |
| `OIDC_ENABLED`                      | Enable Single Sign-On via OpenID Connect (e.g., `true` or `false`)                        | `false`                              |
| `OIDC_ISSUER_URL`                   | IdP issuer URL, must be https (e.g., `https://accounts.google.com`)                       | -                                    |
| `OIDC_CLIENT_ID`                    | OAuth2 client ID from your identity provider                                              | -                                    |
| `OIDC_CLIENT_SECRET`                | OAuth2 client secret (encrypted at rest; never sent to the browser)                       | -                                    |
| `OIDC_REDIRECT_URI`                 | Callback URL registered at the IdP. Derived from `API_ENDPOINT` when empty.               | `<API_ENDPOINT>/api/user.oidc.callback` |
| `OIDC_SCOPES`                       | Space-separated scopes (`openid` always included)                                         | `openid email profile`               |
| `OIDC_BUTTON_LABEL`                 | Sign-in button label                                                                      | `Sign in with SSO`                   |
| `OIDC_AUTO_CREATE_USERS`            | Auto-create accounts on first SSO login (requires `OIDC_ALLOWED_DOMAINS`)                 | `false`                              |
| `OIDC_ALLOWED_DOMAINS`              | Comma/semicolon-separated email-domain allowlist gating auto-creation                     | -                                    |
|                                     |
